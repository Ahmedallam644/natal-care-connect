...
export function DoctorLayout({ children }: DoctorLayoutProps) {
  ...
  const navItems = [
    { path: '/doctor/dashboard', label: language === 'ar' ? 'لوحة التحكم' : 'Dashboard', icon: LayoutDashboard },
    { path: '/doctor/patients', label: language === 'ar' ? 'المريضات' : 'Patients', icon: Users },
    { path: '/doctor/alerts', label: language === 'ar' ? 'التنبيهات' : 'Alerts', icon: AlertTriangle },
    { path: '/doctor/lab-orders', label: language === 'ar' ? 'طلبات التحاليل' : 'Lab Orders', icon: TestTube },
    { path: '/doctor/chat', label: language === 'ar' ? 'المحادثات' : 'Messages', icon: MessageCircle },
    { path: '/doctor/analytics', label: language === 'ar' ? 'التحليلات' : 'Analytics', icon: BarChart3 },
    { path: '/doctor/settings', label: language === 'ar' ? 'الإعدادات' : 'Settings', icon: Settings },
  ];
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 glass border-b border-border">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">{language === 'ar' ? 'بوابة الطبيب' : 'Doctor Portal'}</span>
          </div>
        ...
        <header className="hidden lg:flex items-center justify-between px-8 h-16 border-b border-border bg-card">
          <div>
            <h2 className="font-semibold text-foreground">
              {language === 'ar' ? 'مرحباً' : 'Welcome'}, Dr. {profile?.full_name || 'Doctor'}
            </h2>
          </div>
